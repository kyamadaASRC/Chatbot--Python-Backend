...
| `app/template_helpers/` | `docx_helper.py` / `xlsx_helper.py` / `pdf_helper.py` / `fuzzy_mapper.py` inspect templates, capture placeholder maps, surface fuzzy anchor suggestions, and emit JSON for consultants or UI review. |
| `app/scripts/build_template_reports.py` | Batch script that runs the template helpers for every consultant and stores JSON under `placeholder_reports/`. |
| `app/scripts/suggest_template_anchors.py` | CLI to generate fuzzy anchor proposals (with confidence scores) for any uploaded template so humans can tweak mappings before Code Interpreter runs. |
...
- Each consultant folder now includes a `placeholder_reports/` subdirectory generated by `python -m app.scripts.build_template_reports`. These JSON files list every paragraph/table cell placeholder (both DOCX and XLSX) so Code Interpreter can overwrite the exact strings the template expects. The registry automatically appends a generic directive instructing each consultant to load the relevant JSON (matching filename stem) before editing a template.
- For ad-hoc templates or near matches, run `python -m app.scripts.suggest_template_anchors --template <file> --targets anchors.json`. Under the hood, `app/template_helpers/fuzzy_mapper.py` computes similarity scores so you can review a proposed anchor map (complete with confidence flags) and hand-edit it before routing that data to a consultant.
...
3. The backend either routes to a consultant or to the general assistant. Even in the direct path, the model can emit `call_<consultant>` tool invocations, which the backend fulfills by running that consultant and returning the results.
